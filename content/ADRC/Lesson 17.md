...
In questa sezione vedremo dei metodi per il calcolo distribuito di una $\alpha$-**colorazione** di una rete $G$. In maniera formale, un'*$\alpha$-colorazione* di un grafo è una funzione
$$
...
> [!tldr] Osservazione
> Convincersi del fatto che ogni grafo $\Delta$-regolare accetta sempre una $(\Delta + 1)$-colorazione.

## Message-Passing $\mathcal{LOCAL}$ Model
Nel modello *Message-Passing $\mathcal{LOCAL}$ Model* consideriamo una rete distribuita di $n$ nodi, tutti identificati da un **Unique Id**, e che si possono scambiare dei messaggi (di dimensioni illimitate) in maniera del tutto [[Lesson 2#2. Synchronized clocks|sincrona]].
...
La **Basic Color Reduction Tecnique** consente di ridurre la colorazione da $\alpha$ colori a $\Delta + 1$ in tempo $\alpha - (\Delta + 1)$.
...
Sicuramente nel suo vicinato $N(v)$ ci possono essere **al più** $\Delta$ nodi di colore diverso (dato che $G$ $\Delta$-regolare).
Ovvero
$$
...
$$
\beta \in_U \left[\Delta + 1\right] \setminus \phi\left(N(v)\right)
$$
...
Iterando questo processo per i colori $\alpha - 2, \alpha - 3, ..., \Delta + 2$ otterremo una $(\Delta + 1)$-colorazione legale.
...
Da questa tecnica è anche possibile ricavare un *Independent Set* **Massimale**[^1].
Iniziamo con un insieme $U$ vuoto.
Ad ogni passo $i$, tutti i nodi $v$ di colore $\phi(v) = i$ controllano **in parallelo** se hanno vicini in $U$.
...
Nel caso peggiore però, l'$\alpha$-colorazione iniziale potrebbe essere una biezione in
...
Perciò possiamo applicare la *Basic Color Reduction Tecnique* sul grafo indotto $G(V_1 \cup V_2)$ per ottenere una $(\Delta + 1)$-colorazione $\phi_{12}$ in tempo $(\Delta + 1)$.
Possiamo fare la stessa cosa anche per il sottografo $G(V_3 \cup V_4)$, assicurandoci solamente di far scegliere i colori nell'intervallo $\left[ 3(\Delta + 1), 4(\Delta + 1) \right]$, così da garantire che alla fine il grafo intero $G$ abbia una colorazione accettabile.
Possiamo fare la stessa cosa anche per i sottografi $V_5 \cup V_6, ..., V_{k-1} \cup V_k$ (se $k$ è dispari, allora basterà fare $V_{k-2} \cup V_{k-1} \cup V_k$).

Dato che stiamo in una rete distribuita, possiamo fare tutte queste operazioni **in
parallelo**.
Così facendo avremo ottenuto $\lfloor k/2 \rfloor$ $(\Delta + 1)$-colorazioni $\phi_{12}, \phi_{34}, \phi_{56}, ..., \phi_{k-1,k}$.

È facile convincersi che dopo una iterazione di questa procedura parallela, il grafo intero $G$ avrà
un'$\left(\frac{\alpha}{2}\right)$-colorazione $\phi_{12} \cup \phi_{34} \cup \phi_{56} \cup ... \cup \phi_{k-1,k}$.
...
Perciò, sempre nel caso peggiore in cui $\alpha$ è una biezione in $n$, il running time di questa procedura sarà
$$
O\left( \Delta \log{\left( \frac{\alpha}{\Delta} \right)}\right) \in O\left( \Delta \log{\left( \frac{n}{\Delta} \right)}\right)
$$


[^1]: massimale indica un **massimo locale**, e non globale.
