...
Nella [[Lesson 10|lezione 10]] è stato introdotto il protocollo `Round-Robin` per il task del *broadcast* a singola sorgente su reti wireless **unknown**[^1] con presenza d'interferenza, ed è stato dimostrato che questo protocollo porta a termine il suo *task* in tempo $O(Dn)$, dove $D$ è il **diametro** della rate di $n$ nodi considerata.

Avvalendosi di altri costrutti matematici e aggiungendo conoscenza ai nodi è possibile ottenere risultati migliori.
Per esempio, dato l'insieme $\left[ n \right]$ e un $k \leq n$ diremo che la famiglia di sottoinsiemi $H = \lbrace H_1, H_2, ..., H_t \rbrace$ è $(n,k)$-*selettiva* se per ogni sottoinsieme $S \subseteq \left[ n \right]$ con $\vert S \vert \leq k$ esiste almeno un $H_i \in H$ tale che $\vert S \cap H_i \vert = 1$.

Se supponiamo che tutti i nodi conoscano la stessa famiglia $H$ $(n,\Delta)$-selettiva di $V$, dove $\Delta = \max_{v \in V}{ \lbrace d(v) \rbrace }$, allora è possibile definire un protocollo deterministico che termina il task in questione in tempo $O(D \cdot \vert H \vert)$, con $D = diam(G)$.

Nell'articolo [Round Robin is optimal for fault-tolerant broadcasting on wireless networks](https://dl.acm.org/doi/10.1016/j.jpdc.2003.09.002) viene dimostrato che per valori sufficientemente grandi di $n$ esiste sempre una famiglia $(n,k)$-selettiva di dimensione $O(k \log{n})$.
...
> [!theorem]
> Data un grafo diretto di trasmissione $G=(V,E)$ relativo ad una rete wireless, con diametro $D$, grado massimo $\Delta$ tale che $D\Delta < n$, allora esiste sempre un modo di manipolare i ritardi in modo tale che non si possa risolvere il problema del broadcasting a singola sorgente in un tempo migliore di
> $$
...
Il [[#^98ea2f|teorema precedentemente enunciato]] ci dice che questo è **deterministicamente** impossibile, perciò è necessario occorrere alla *randomness*.


----------
# Randomized protocol for Unknown Radio Networks with Interferences

Definiamo un protocollo probabilistico molto semplice, in cui ogni nodo informato trasmette ai suoi vicini con una certa probabilità $p$ fissata e uguale per tutti.
È facile mostrare un controesempio in cui il tempo medio di completamento del protocollo è pessimo.

- **Figura:** Controesempio.

	![[adrc-lesson12-img1.png|400]]

Sia la sorgente $u$. Sicuramente esiste un tempo $t_0$ in cui $u$ trasmette il messaggio a tutti i suoi vicini $1, 2, ..., n-1$ senza interferenze.
Dal time slot successivo tutti i suoi vicini inizieranno a trasmettere verso $v$, ognuno con probabilità $p$.
...
Per prima cosa dobbiamo calcolare con quale probabilità l'evento $\mathcal{E} = \text{``} v \text{ vinene informato"}$ occorre, e questo si può fare usando la distribuzione binomiale
$$
...
Se invece $v$ avesse solamente $2$ vicini che trasmettono, il protocollo terminerebbe in tempi brevissimi (sempre considerando questo esempio).
...
> 
> Il caso base per $j = 1$ è banale, perciò assumiamo come *ipotesi induttiva* che **con alta probabilità** entro la fine dello stage $j > 1$ tutti i nodi nel livello $L_j$ sono già stati informati (ovvero entro $O(j\log{n})$ time slots).
...
> $$
>
> Ricapitolando abbiamo dimostrato che la probabilità dell'evento $\mathcal{E}_v = \text{``} v \in L_{j + 1} \text{ non viene informato entro la fine dello stage } j+1 \text{"}$ occorre con al più come
> $$
> \mathcal{P}(\mathcal{E}_v) \leq \frac{1}{n^{\frac{c}{8}}}
> $$
> 
> Per **union bound**, possiamo dire che esiste **almeno un nodo** al livello $L_{j+1}$ che non viene informato entro la fine dello stage $j + 1$ occorre con probabilità al più
> $$
> \mathcal{P}\left(\bigcup_{v \in L_{j+1}} \mathcal{E}_v \right) \leq \sum_{v \in L_{j+1}} \mathcal{P}(\mathcal{E}_v) < n \frac{1}{n^{\frac{c}{8}}} = \frac{1}{n^{\frac{c}{8} - 1}}
> $$
>
...
> Possiamo quindi concludere dicendo che ogni nodo $v \in L_{j+1}$ viene informato entro la fine dello stage $j + 1$ con probabilità **almeno** $1 - \frac{1}{n}$, implicando così il teorema $\square$.

[^1]: reti in cui i non hanno nessuna informazione della rete, nemmeno la dimensione oppure il proprio vicinato.
