...
\end{align*}$$
viceversa la probabilità che la scelta corretta sia `N` ricevendo il segnale `A` è $$\mathcal{P}(N \mid A) = 1 - \mathcal{P}(Y \mid A) < p$$
In maniera simmetrica, se riceviamo come primo segnale privato `R` avremo che $$\mathcal{P}(Y \mid R) < p \;;\;\;\mathcal{P}(N \mid R) > p$$
